...
export const getWeeklySets = vi.fn((muscle) =>
  baseState.volumeLandmarks[muscle]?.current ?? 0,
);
...
export const getTotalWeeklyVolume = vi.fn(() =>
  Object.values(baseState.volumeLandmarks)
    .reduce((sum, m) => sum + (m.current ?? 0), 0),
);
...
export function resetMockState() {
  ...
  Object.keys(baseState.volumeLandmarks).forEach((m) => {
    baseState.volumeLandmarks[m].current =
      baseState.volumeLandmarks[m].MEV;
  });
}
