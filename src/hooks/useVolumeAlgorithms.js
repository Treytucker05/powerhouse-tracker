...
export const useVolumeAlgorithms = () => {
    ...
    /**
     * Generate volume progression recommendations
     */
    const generateVolumeProgression = useCallback((currentVolume, targetVolume, weeksPlan = 4) => {
        if (!currentVolume || !targetVolume || weeksPlan <= 0) {
            return null;
        }

        const volumeDiff = targetVolume - currentVolume;
        const weeklyIncrement = volumeDiff / weeksPlan;

        const progression = [];
        for (let week = 1; week <= weeksPlan; week++) {
            const weekVolume = Math.round(currentVolume + (weeklyIncrement * week));
            progression.push({
                week,
                volume: weekVolume,
                increment: week === 1 ? weekVolume - currentVolume : Math.round(weeklyIncrement)
            });
        }
        ...
        const result = {
            ...
            targetVolume,
            weeksPlan,
            weeklyIncrement: Math.round(weeklyIncrement),
            progression,
            totalIncrease: volumeDiff
        };
    ...
    /**
     * Analyze set quality based on RIR data
     */
