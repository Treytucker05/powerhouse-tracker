...
import type { TemplateNormalized } from '../../../lib/templates/types';
...
export default function Step2TemplateGallery({ onSelect, autoNext }: Props) {
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [templates, setTemplates] = useState<TemplateNormalized[]>([]);
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
        {list.map(t => {
          ...
          return (
            <button
              key={t.id}
              onClick={() => { onSelect?.(t.id); autoNext?.(); }}
