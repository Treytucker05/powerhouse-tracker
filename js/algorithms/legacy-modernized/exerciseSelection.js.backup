...
/**
 * Add new exercise category with Bryant cluster support
 * @param {string} type - Exercise type ('cluster', 'standard', 'complexes')
 * @param {Object} params - Exercise parameters
 * @returns {Object} - Updated EXERCISE_DATABASE
 */
function addExerciseCategory(type, params) {
  ...
  // Initialize muscle group if doesn't exist
  if (!EXERCISE_DATABASE[muscleGroup]) {
    EXERCISE_DATABASE[muscleGroup] = {};
  }

  // Bryant cluster configuration
  const clusterConfig = type === 'cluster' ? {
    clusterType: 'bryant_periodization',
    intraRest: 15, // 15 seconds between mini-sets
    clustersPerSet: 3, // 3 mini-sets per cluster
    repsRange: [3, 5], // 3-5 reps per mini-set
    totalRepsFormula: 'sets * (repsPerCluster * clustersPerSet)',
    effectiveVolumeFormula: 'totalReps * (1 - (intraRest / 60))',
    bryantCompliant: true
  } : {};

  // Bryant strongman configuration
  const strongmanConfig = type === 'strongman' ? {
    strongmanType: 'bryant_periodization',
    timeBased: true,
    distance: params.distance || 150, // Default 150ft
    duration: params.duration || 30, // Default 30s
    restBetweenEvents: params.rest || 90, // Default 90s rest
    distanceFormula: 'estimated_reps = distance / 5',
    volumeFormula: 'estimated_reps * load_factor',
    loadFactor: params.loadFactor || 1.2, // Strongman intensity multiplier
    bryantCompliant: true,
    tacticalApplication: true
  } : {};

  // Add exercise to database
  EXERCISE_DATABASE[muscleGroup][exerciseName] = {
    type: type,
    primaryMuscles,
    secondaryMuscles,
    equipment,
    ...
    skillRequirement,
    ranges: type === 'cluster' ?
      { strength: [3, 5], hypertrophy: [3, 5], endurance: [4, 6] } :
      type === 'strongman' ?
        { tactical: [20, 40], endurance: [30, 60], power: [10, 30] } :
        { strength: [1, 5], hypertrophy: [6, 12], endurance: [12, 20] },
    bryantFlag,
    ...clusterConfig,
    ...strongmanConfig
  };
...
// Add sample Bryant strongman exercises
addExerciseCategory('strongman', {
...
// Add sample Bryant cluster exercises
...
addExerciseCategory('strongman', {
  muscleGroup: 'full_body',
  exerciseName: 'farmers_walk',
  equipment: ['dumbbells', 'farmers_walk_handles'],
  fatigueIndex: 8.0,
  skillRequirement: 4,
  primaryMuscles: ['traps', 'forearms', 'core'],
  secondaryMuscles: ['quads', 'glutes', 'calves'],
  ...
  rest: 90, // 90 seconds between events
  loadFactor: 1.3
});
...
addExerciseCategory('strongman', {
  ...
  secondaryMuscles: ['shoulders', 'triceps', 'core'],
  bryantFlag: true,
  distance: 100, // 100 feet
  duration: 45, // 45 seconds
  rest: 120, // 2 minutes between events
  loadFactor: 1.5
});
...
addExerciseCategory('strongman', {
  ...
  secondaryMuscles: ['biceps', 'core', 'quads'],
  bryantFlag: true,
  distance: 0, // No distance component
  duration: 60, // 60 second events
  rest: 180, // 3 minutes between events
  loadFactor: 1.8
});
