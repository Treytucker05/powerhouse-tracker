...
describe("LogSet Algorithm", () => {
  ...
  describe("logSet validation", () => {
    test("should throw error when no session provided", () => {
      ...
      expect(() => logSet(null, setData)).toThrow(
        "No active workout session provided",
      );
    });
    ...
    test("should throw error when session is not active", () => {
      ...
      expect(() => logSet(mockSession, setData)).toThrow(
        "Cannot log set - workout session is not active",
      );
    });

    test("should throw error when setData is invalid", () => {
      expect(() => logSet(mockSession, null)).toThrow(
        "Invalid set data provided",
      );
      expect(() => logSet(mockSession, undefined)).toThrow(
        "Invalid set data provided",
      );
      expect(() => logSet(mockSession, "string")).toThrow(
        "Invalid set data provided",
      );
    });
    ...
    test("should throw error when exercise name is missing", () => {
      ...
      const setDataEmptyExercise = {
        exercise: "",
        weight: 135,
        reps: 10,
        rir: 2,
      };
      expect(() => logSet(mockSession, setDataEmptyExercise)).toThrow(
        "Exercise name is required",
      );
    });
    ...
    test("should throw error when weight is invalid", () => {
      ...
      expect(() =>
        logSet(mockSession, { ...baseSetData, weight: undefined }),
      ).toThrow("Valid weight is required");
      expect(() =>
        logSet(mockSession, { ...baseSetData, weight: null }),
      ).toThrow("Valid weight is required");
      expect(() => logSet(mockSession, { ...baseSetData, weight: -5 })).toThrow(
    ...
    test("should throw error when reps is invalid", () => {
      ...
      expect(() =>
        logSet(mockSession, { ...baseSetData, reps: undefined }),
      ).toThrow("Valid reps count is required");
      expect(() => logSet(mockSession, { ...baseSetData, reps: null })).toThrow(
        "Valid reps count is required",
      );
      expect(() => logSet(mockSession, { ...baseSetData, reps: -1 })).toThrow(
        "Valid reps count is required",
      );
    });
    ...
    test("should throw error when RIR is out of range", () => {
      ...
      expect(() => logSet(mockSession, { ...baseSetData, rir: -1 })).toThrow(
        "RIR must be between 0 and 10",
      );
      expect(() => logSet(mockSession, { ...baseSetData, rir: 11 })).toThrow(
        "RIR must be between 0 and 10",
      );
    });
    ...
    test("should allow valid RIR values", () => {
      ...
      expect(() =>
        logSet(mockSession, { ...baseSetData, rir: 0 }),
      ).not.toThrow();
      expect(() =>
        logSet(mockSession, { ...baseSetData, rir: 5 }),
      ).not.toThrow();
      expect(() =>
        logSet(mockSession, { ...baseSetData, rir: 10 }),
      ).not.toThrow();
    });
  ...
  describe("logSet functionality", () => {
    ...
    test("should handle multiple exercises", () => {
      ...
      expect(updatedSession.exercises[1].muscleGroups).toEqual([
        "quadriceps",
        "glutes",
      ]);
    });
    ...
    test("should update session totals correctly", () => {
      ...
      const updatedSession = logSet(mockSession, {
        exercise: "Squat",
        weight: 185,
        reps: 8,
        rir: 3,
      ...
      expect(updatedSession.muscleGroups).toEqual(
        expect.arrayContaining(["chest", "triceps", "quadriceps", "glutes"]),
      );
    });
