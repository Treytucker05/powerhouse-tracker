...
function updateBasicStatus() {
  const elements = {
    ...
    currentMeso: trainingState.getAdaptiveMesoLength
...
export function updateFatigueIndicator() {
  try {
    ...
    if (fatigueIndicator) {
      ...
      fatigueIndicator.classList.remove(
        "fatigue-low",
        "fatigue-moderate",
        "fatigue-high",
      );
    ...
    if (fatigueWarning) {
      if (fatiguePercentage >= 80) {
        ...
        fatigueWarning.textContent =
          fatiguePercentage >= 90
...
function updateAdaptiveMesoIndicator() {
  ...
  if (mesoElement && trainingState.getAdaptiveMesoLength) {
    ...
    if (adaptiveLength !== baseLength) {
      ...
      mesoElement.title =
        "Adaptive mesocycle length based on progression patterns";
    } else {
