...
class ExperienceToggle {
  ...
  initialize() {
    ...
    setTimeout(() => {
      ...
      this.loadSavedLevel();
      console.log(
        "🎯 Experience toggle initialized at level",
        this.currentLevel,
      );
    }, 100);
  }
  ...
  updateLevel(level, save = true) {
    ...
    if (oldLevel !== this.currentLevel) {
      console.log(
        `🎯 Experience level changed: ${oldLevel} → ${this.currentLevel}`,
      );
  ...
  trackLevelChange(oldLevel, newLevel) {
    ...
    if (phaseSections) {
      ...
      console.log(
        `📊 Phase visibility: ${stats.visiblePhases}/${stats.totalPhases} phases visible`,
      );
    }
  }
    ...
    [...document.querySelectorAll("select#experienceToggle")]
      ...
      .forEach((el) => {
        ...
        if (
          label &&
          label.tagName === "LABEL" &&
          label.innerText.trim() === "EXPERIENCE:"
        ) {
          label.remove();
        }
        el.remove();
