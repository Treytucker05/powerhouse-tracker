...
const PowerHouseVolumeChart = ({ className = '' }) => {
  ...
  const CustomTooltip = ({ active, payload, label }) => {
    if (active && payload && payload.length) {
      ...
      const { status } = getVolumeStatus({ volume: data.volume, mev: data.mev, mrv: data.mrv });
      ...
      return (
        <div className="bg-gray-950 border border-accent p-3 rounded-lg shadow-lg">
          ...
          <p className="text-green-400">Volume: {data.volume} sets</p>
          <p className="text-yellow-400">MEV: {data.mev} sets</p>
          <p className="text-primary">MRV: {data.mrv} sets</p>
          <p className={`font-bold ${status === 'within' ? 'text-green-400' : 'text-accent'}`}>Status: {pretty}</p>
        </div>
      );
    }
    return null;
    ...
    <div className={`bg-gray-900 p-6 rounded-2xl border border-gray-700 shadow-lg ${className}`}>
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center space-x-4 text-sm">
          <div className="flex items-center space-x-2"><div className="w-3 h-3 bg-green-500 rounded-full"></div><span className="text-gray-300">Optimal</span></div>
          <div className="flex items-center space-x-2"><div className="w-3 h-3 bg-accent rounded-full"></div><span className="text-gray-300">Sub-optimal</span></div>
          <div className="flex items-center space-x-2"><div className="w-4 h-0.5 bg-yellow-500"></div><span className="text-gray-300">MEV</span></div>
          <div className="flex items-center space-x-2"><div className="w-4 h-0.5 bg-primary"></div><span className="text-gray-300">MRV</span></div>
        </div>
      </div>
      <ResponsiveContainer width="100%" height={500}>
        <BarChart data={chartData} margin={{ top: 20, right: 30, left: 40, bottom: 80 }}>
          <CartesianGrid strokeDasharray="3 3" stroke="#374151" />
          <XAxis dataKey="muscle" stroke="#f2f2f2" tick={{ fill: '#f2f2f2', fontSize: 12 }} angle={-45} textAnchor="end" height={80} />
          <YAxis stroke="#f2f2f2" tick={{ fill: '#f2f2f2', fontSize: 12 }} label={{ value: 'Sets', angle: -90, position: 'insideLeft', style: { textAnchor: 'middle', fill: '#f2f2f2' } }} />
